...
def createLegends():
    ...
    legend_equation[3] = "alpha_N" # "(dimensionless)"
    legend_equation[4] = "beta_N" # "(dimensionless)"
    legend_equation[5] = "R_N" #1 + alpha_n + beta_n + alpha_n * beta_n
    legend_equation[6] = "R_MM" #g_EA_M*alpha_m + g_EB_M*beta_M
    legend_equation[7] = "R_M" #1 + alpha_m + beta_m + alpha_m * beta_m
    legend_equation[8] = "R_NN" #g_EB_N*beta_N + g_EA_N*alpha_N
    legend_equation[9] = "JDenominator" #R_M*R_NN + R_N*R_MM
    legend_equation[10] = "J_AFirst_Numerator" #(g_EA_M*alpha_M)*(g_EB_N*beta_N)
    legend_equation[11] = "J_ASecond_Numerator" #(g_EA_N*alpha_N)*(g_EB_M*beta_M)
    legend_equation[12] = "J_ANumerator"
...
# 5) R_N = equation[5] = 1 + alpha_n + beta_n + alpha_n * beta_n  
	equation[5] = (1.00000 + equation[3] + equation[4] + equation[3] * equation[4])
# 6) R_MM = equation[6] = g_EA_M*alpha_m + g_EB_M*beta_M
	equation[6] = (parameters[7]*equation[1] + parameters[5]*equation[2])
# 7) R_M = equation[7] = 1 + alpha_m + beta_m + alpha_m * beta_m
	equation[7] = (1.00000 + equation[1] + equation[2] + equation[1] * equation[2])
...
'Et':Et,'J_AMN':J_AMN, 'J_BMN':J_BMN,'alphaM':alphaM,'alphaN':alphaN,'betaM':betaM,'betaN':betaN,\
...
'J_AMN':J_AMN, 'J_BMN':J_BMN})

#print("J_A = " , equation[0])
#np.savetxt("foo.csv", Et, delimiter=",")
###### output as csv file or table as well
column_order = ['time', 'A_int(mM)', 'A_ext(mM)','B_int(mM)','B_ext(mM)',\
'J_AMN', 'J_BMN','Et','alphaM','betaM',
'alphaN','betaN','R_N','R_MM','R_M','R_NN',\
...
#	df.to_csv(str(out3) + '.csv', sep = ',')
	df[column_order].to_csv(str(out3) + '_AntiEightState' +'.csv', sep = ',',index=False)
	print('The new csv file has been saved under the name of ' + str(out3) + '_AntiEightState' +'.csv')
else:
	print('Ok, Continue')
...
def plot_model(time, ICs, equation):
    ...
    plt.savefig(str(out6) + '_AntiEightState' + '.png')
    plt.savefig(str(out6) + '_AntiEightState',format = 'eps', dpi=1000, bbox_inches='tight')
    plt.show()
if __name__ == "__main__":
	plot_model(time, ICs, equation)
