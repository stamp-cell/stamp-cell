...
etha = input('Enter the Hill coeffiecient (etha): ') 
K_pump = input('Enter K_pump (mM) ') 
def createLegends():
	...
	legend_parameters[0]  = "J_APump_Max in component NaK (mM_per_s)" 
	legend_parameters[2] = "etha"
	legend_parameters[3] = "K_pump"
	legend_equation[0] = "J_A_Pump (mM_per_s)"
...
def initConsts():
	...
	parameters[0] = float(J_A_Max) # 10.8 (mM_per_s)
	parameters[1] = 1.00000 # d/dt Na_int in component concentrations (mM)
	parameters[2] = float(etha)
	parameters[3] = float(K_pump)
...
def computeequation(parameters, ICs, time):
	...
	ICs = array(ICs)
	time = array(time)
# 0) J_A,pump = J_A,pump,Max *(A_M^etha/(A_M^eta + K_pump^etha)) 
	equation[0] = parameters[0]*(power(ICs[0], parameters[2]))/((power(ICs[0], parameters[2]))+(power(parameters[3], parameters[2])))
	return equation
...
#	df.to_csv(str(out3) + '.csv', sep = ',')
	df[column_order].to_csv(str(out3) + '_pumphill' +'.csv', sep = ',',index=False)
	print('The new csv file has been saved under the name of ' + str(out3) + '_pumphill' +'.csv')
else:
	print('Ok, Continue')
...
def plot_model(time, ICs, equation):
    ...
    plt.savefig(str(out6) + '_pumphill'+ '.png')
    plt.savefig(str(out6) + '_pumphill',format = 'eps', dpi=1000, bbox_inches='tight')
    plt.show()
if __name__ == "__main__":
