...
def createLegends():
	...
	legend_parameters[0] = "K_A_M (mM)" 
	legend_parameters[1] = "K_AB_M (mM)"
	legend_parameters[2] = "K_A_N (mM)" 
	legend_parameters[3] = "K_AB_N (mM)"
	...
	legend_equation[1] = "alpha_M (dimensionless)" # gamma_int in component AE1 (dimensionless)
	...
	legend_equation[13] = "J_A(M, N), J_A_influx in component AE1 (mM_per_s)"
	legend_equation[14] = "J_B(M, N), J_B_influx in component AE1 (mM_per_s)"
...
# Model Parameters:
	...
	parameters[2] = float(K_An) 
	parameters[3] = float(K_ABn) 
...
# 5) R_N = 1 + alpha_n + alpha_n*beta_n
	equation[5] = (1.00000 + equation[3] + equation[3]*equation[4])
# 6) R_MM = g_E_M + g_EAB_M*alpha_M*beta_M
	equation[6] = (parameters[6] + parameters[7]*equation[1]*equation[2])
# 7) R_M = 1 + alpha_m + alpha_M*beta_M
	equation[7] = (1.00000 + equation[1] + equation[1]*equation[2])
# 8) R_NN = g_E_N + g_EAB_N*alpha_N*beta_N
...
# 10) J_A First_Numerator: = (g_EAB_M*g_E_N*beta_M*alpha_M)
	equation[10] =  (parameters[7]*parameters[4]*equation[2]*equation[1])
# 11) J_A Second_Numerator: = (g_EAB_N*g_E_M*alpha_N*beta_N)
	equation[11] =  (parameters[5]*parameters[6]*equation[3]*equation[4])
# 12) J_A Numerator:
...
#	df.to_csv(str(out3) + '.csv', sep = ',')
	df[column_order].to_csv(str(out3) + '_SymAFirst' +'.csv', sep = ',',index=False)
	print('The new csv file has been saved under the name of ' + str(out3) + '_SymAFirst' +'.csv')
else:
	print('Ok, Continue')
...
def plot_model(time, ICs, equation):
    ...
    plt.savefig(str(out6) + '_SymAFirst'+ '.png')
    plt.savefig(str(out6) + '_SymAFirst',format = 'eps', dpi=1000, bbox_inches='tight')
    plt.show()
if __name__ == "__main__":
