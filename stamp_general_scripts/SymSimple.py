...
def createLegends():
	...
	legend_parameters[0] = "K_A (mM)" 
	legend_parameters[1] = "K_B (mM)" 
	legend_parameters[2] = "K_C (mM)"
	legend_parameters[3] = "A stoichiometric"
	legend_parameters[4] = "B stoichiometric"	
	legend_parameters[5] = "C stoichiometric"
	legend_parameters[6] = "g_symporter"
	legend_parameters[7] =" E_t_Tmax (mM)"
	legend_equation[0] = "p_Symporter" # E_t*g_symp
	...
	legend_equation[7] = "J_Symp First_Denominator" # (1+alpha_N)
	legend_equation[8] = "J_Symp Second_Denominator" # (1+beta_N)
	legend_equation[9] = "J_Symp Third_Denominator" # (1+gamma_N)
	legend_equation[10] = "JDenominator" 
	legend_equation[11] = "J_symp_First_Numerator"
	legend_equation[12] = "J_symp_Second_Numerator"
	legend_equation[13] = "J_symp_Numerator"
	legend_equation[14] = "J_symporter(M, N) antiporter influx in component AE1 (mM_per_s)"
	legend_equation[15] = "J_A(M, N) A_influx in component AE1 (mM_per_s)"
	legend_equation[16] = "J_B(M, N) B_influx in component AE1 (mM_per_s)"
	legend_equation[17] = "J_C(M, N) C_influx in component AE1 (mM_per_s)"
...
#Equations:
p_Symporter = pd.Series(equation[0]) # E_t*g_anti
...
J_SympFirst_Denominator = pd.Series(equation[7])
J_SympSecond_Denominator = pd.Series(equation[8])
J_SympThird_Denominator = pd.Series(equation[9])
JDenominator = pd.Series(equation[10]) 
J_symp_First_Numerator = pd.Series(equation[11])
J_symp_Second_Numerator = pd.Series(equation[12])
J_symp_Numerator = pd.Series(equation[13])
J_symporterMN = pd.Series(equation[14])
J_AMN = pd.Series(equation[15])
J_BMN = pd.Series(equation[16])
J_CMN = pd.Series(equation[17])
df = pd.DataFrame({'time':time, 'A_int(mM)':AM, 'A_ext(mM)':AN, 'B_int(mM)':BM,'B_ext(mM)' :BN,\
'C_int(mM)':CM, 'C_ext(mM)':CN, 'p_Symporter':p_Symporter, 'J_symporterMN':J_symporterMN,\
'J_AMN':J_AMN, 'J_BMN':J_BMN,'J_CMN':J_CMN,'alphaM':alphaM,'alphaN':alphaN,'betaM':betaM,'betaN':betaN,'gammaM':gammaM,'gammaN':gammaN,\
'J_SympFirst_Denominator':J_SympFirst_Denominator,'J_SympSecond_Denominator':J_SympSecond_Denominator , 'J_SympThird_Denominator':J_SympThird_Denominator, 'JDenominator':JDenominator, 'J_symp_First_Numerator':J_symp_First_Numerator,\
'J_symp_Second_Numerator':J_symp_Second_Numerator, 'J_symp_Numerator':J_symp_Numerator})
...
'J_symporterMN','J_AMN', 'J_BMN','J_CMN','p_Symporter','alphaM','betaM',
'alphaN','betaN','gammaM','gammaN','J_SympFirst_Denominator','J_SympSecond_Denominator', 'J_SympThird_Denominator',\
'JDenominator','J_symp_First_Numerator','J_symp_Second_Numerator','J_symp_Numerator']
...
#	df.to_csv(str(out3) + '.csv', sep = ',')
	df[column_order].to_csv(str(out3) + '_SymSimple' +'.csv', sep = ',',index=False)
	print('The new csv file has been saved under the name of ' + str(out3) + '_SymSimple' +'.csv')
else:
	print('Ok, Continue')
...
def plot_model(time, ICs, equation):
    ...
    plt.savefig(str(out6) + '_SymSimple' + '.png')
    plt.savefig(str(out6) + '_SymSimple',format = 'eps', dpi=1000, bbox_inches='tight')
    plt.show()
if __name__ == "__main__":
	plot_model(time, ICs, equation)
