...
def createLegends():
	...
	legend_parameters[2] = "K_ACm (mM)"
	legend_parameters[3] = "K_An (mM)" 
	legend_parameters[4] = "K_ABn (mM)" 
	legend_parameters[5] = "K_ACn (mM)"
	legend_parameters[6] = "g_E_N (per_s)" 
	legend_parameters[7] = "g_EAC_N (per_s)" 
	legend_parameters[8] = "g_EAB_N (per_s)"
	legend_parameters[9] = "g_E_M (per_s)" 
	legend_parameters[10] = "g_EAC_M (per_s)"
	legend_parameters[11] = "g_EAB_M (per_s)"
	...
	legend_equation[7] = "R_N" # 1 + alpha_N + alpha_N*beta_N + alpha_N*gamma_N
	legend_equation[8] = "R_MM" # g_E_M + g_EAB_M*alpha_M*beta_M + g_EAC_M*alpha_M*gamma_M
	legend_equation[9] = "R_M" # 1 + alpha_M + alpha_M*beta_M + alpha_M*gamma_M
	legend_equation[10] = "R_NN" # g_E_N + g_EAB_N*alpha_N*beta_N + g_EAC_N*alpha_N*gamma_N
	legend_equation[11] = "J Denominator" # R_M*R_NN + R_N*R_MM
	legend_equation[12] = "J_B First_Numerator" # R_NN*(g_EAB_M*alpha_M*beta_M)
	legend_equation[13] = "J_B Second_Numerator" # R_MM*(g_EAB_N*alpha_N*beta_N)
	legend_equation[14] = "J_B Numerator"
	legend_equation[15] = "J_B(M, N), B_influx (mM_per_s)"
	legend_equation[16] = "J_C First_Numerator" # R_NN*(g_EAC_M*alpha_M*gamma_M)
	legend_equation[17] = "J_C Second_Numerator" # R_MM*(g_EAC_N*alpha_N*gamma_N)
	legend_equation[18] = "J_C Numerator"
	legend_equation[19] = "J_C(M, N), C_influx (mM_per_s)"
	legend_equation[20] = "J_A(M, N), A_influx (mM_per_s)"
...
J_BFirst_Numerator = pd.Series(equation[12])
J_BSecond_Numerator = pd.Series(equation[13])
J_BNumerator = pd.Series(equation[14])
J_CFirst_Numerator = pd.Series(equation[16])
J_CSecond_Numerator = pd.Series(equation[17])
J_CNumerator = pd.Series(equation[18])
J_AMN = pd.Series(equation[20])
J_BMN = pd.Series(equation[15])
...
'R_N':RN,'R_MM':RMM , 'R_M':RM, 'R_NN':RNN, 'JDenominator':JDenominator, 'J_BFirst_Numerator':J_BFirst_Numerator,\
...
'JDenominator','J_BFirst_Numerator','J_BSecond_Numerator','J_BNumerator',\
...
#	df.to_csv(str(out3) + '.csv', sep = ',')
	df[column_order].to_csv(str(out3) + '_SymCompete' +'.csv', sep = ',',index=False)
	print('The new csv file has been saved under the name of ' + str(out3) + '_SymCompete' +'.csv')
else:
	print('Ok, Continue')
...
def plot_model(time, ICs, equation):
    ...
    plt.savefig(str(out6) + '_SymCompete' + '.png')
    plt.savefig(str(out6) + '_SymCompete',format = 'eps', dpi=1000, bbox_inches='tight')
    plt.show()
if __name__ == "__main__":
	plot_model(time, ICs, equation)
